...
| `mods/breath/mic.js` | マイク観測基盤（`robot.microphone` から約 100ms 窓の rms・peak を連続計算。レベル値のみ扱い音声内容は扱わない — 受動原則。UDP port 8688 へストリーム + `GET /mic` / `PUT /mic/params`。v1.1.0 Phase 3a） |
