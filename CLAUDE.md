...
| `overlay/` | breathing 固有の MOD・スクリプト・ドキュメント（ファーム本体の変更は `stack-chan/` 内で直接コミットする） |
...
| `mods/breath/mic.js` | マイク観測基盤 + イベント検出（`robot.microphone` から約 100ms 窓の rms・peak を連続計算し、loud/clap/voice/silence を検出。レベル値と特徴量のみ扱い音声内容は扱わない — 受動原則。UDP port 8688 へストリーム + `GET /mic` / `PUT /mic/params` + `onMicEvent(cb)` 購読。cry・settings-bar ビープとは suspendCapture/resumeCapture ハンドシェイク。v1.1.0 Phase 3a/3b） |
| `mods/breath/param-store.js` | パラメータ永続化の共有モジュール（mergeValidated / クランプ / Preference。mic.js が使用。liveliness.js は未移行） |
...
| `scripts/mic-monitor.sh` | Mac 側マイクレベル受信（port 8688、rms を `#` バー表示。`PUT /mic/params` で `stream.enabled: true` にすると流れてくる） |

### fork ブランチ運用（stack-chan submodule）

`stack-chan/` は **fork [hirorocky/stack-chan](https://github.com/hirorocky/stack-chan) の `breath` ブランチ**を指す（`.gitmodules` で固定）。旧来の「パッチファイルを `git apply` + dirty submodule」方式は廃止し、ファームウェア変更は submodule 内で直接編集・コミットする。
...
deploy マニフェストの原本は `stack-chan/firmware/stackchan/manifest_breath_deploy.json`、MOD 書き込みラッパーの原本は `stack-chan/firmware/scripts/mod-cores3.sh`（どちらも fork 内。breathing 側へコピーする手順は無くなった）。
...
2. **`stack-chan/` は fork（hirorocky/stack-chan）の `breath` ブランチ。** ファーム変更は submodule 内で直接編集する。コミット・push はユーザー明示時に submodule → breathing の 2 段で行い（前節「fork ブランチ運用」参照）、upstream（本家）へは push しない。
...
./scripts/stack-chan-setup.sh   # 未初期化時のみ submodule を checkout。案内メッセージを表示
cd stack-chan/firmware
npm install
npm run setup -- --device=esp32
```

upstream 更新後は「fork ブランチ運用」節の upstream 取り込み手順（`fetch upstream` → `merge upstream/develop` → ビルド確認 → `push origin breath`）を使う。
...
```
main.ts
  1. defaultMod = import 'default-mods/mod'   ← stack-chan 内で breath/mod を指すよう直接編集済み
...
- `stack-chan/firmware/stackchan/manifest_breath_deploy.json` に `"breathHostMod": true`
- `main.ts`（fork 内で直接編集）が MOD パーティション上書きをスキップ
- `default-mods/mod.ts`（fork 内で直接編集）が `import breathMod from 'breath/mod'`（makefile 競合回避）
...
1. `manifest_breath_deploy.json`（fork 内）に `"breath/mod": "../../../overlay/mods/breath/mod"` を登録
2. `default-mods/mod.ts`（fork 内）から `import breathMod from 'breath/mod'`

manifest で `"default-mods/mod": breath` だけ指定しても、makefile 上 **`default-mods/mod.ts` の glob ルールが後から勝つ**ため効かない。`mod.ts` 側の import が必須。
...
後者が勝つと `on-robot-created.ts`（ドロワー UI）が使われる。**`mod.ts` から breath/mod を import する（fork 内で対応済み）のが正解。**
...
breath 用 JS（`overlay/mods/breath/`）は `stack-chan/firmware/stackchan/manifest_breath_deploy.json` 経由で **ホストファームウェアに焼き込まれる**。`mcconfig` の増分ビルドが `overlay/` の変更を拾わず、**更新前の `xs_esp32.bin` のまま `deploy` が走る**ことがある。
...
- `mod.js` の段階起動: ステータスバー・設定バー（~2s、`applySavedSettings()` → `attachSettingsBar()`）→ dev ツール（~3s、`config.breathDevTools` 時）→ cry 初期化（~4s）→ liveliness（~5s）→ mic 観測基盤（~6s）。touch-debug は未接続
...
`overlay/mods/breath/dev/`（trace-udp / dev-server / dev-tools / beacon）。`stack-chan/firmware/stackchan/manifest_breath_deploy.json` の `"breathDevTools": true` で有効。
...
**日常の breath 反復開発の主経路。** `stack-chan/firmware/stackchan/manifest_breath_deploy.json` の `"defines": {"ota": {"autosplit": 1}}` でパーティションを `ota_0`/`ota_1`/`otadata` + `xs`（mod）/`storage`/`nvs` に再構成済み（実機確認済み・2026-07-06）。パーティション未移行の機体では先に USB フル書き込みが必要（前節）。
...
- `mcrun -d` が `Installing mod..` でハングしやすい → `mod-cores3.sh`（fork 内 `stack-chan/firmware/scripts/mod-cores3.sh`。esptool リセット + 最大 3 回リトライ）
- **breath 探求の主経路はフル deploy**（`deploy:breath`）。MOD 単体だけではホスト側 breath 組み込みが効かない
...
| 画面真っ白 | `breath/mod` 未登録・import 失敗 | fork 内 manifest + `default-mods/mod.ts` の import を確認 |
...
| `PUT /ota` が 401 | `x-dev-token` 不一致 | `stack-chan/firmware/stackchan/manifest_breath_deploy.json` の `config.devToken`（既定 `breath-dev`）とヘッダを揃える |
...
| マイクがステレオにならない（方向推定の l0・l1 が同一/lag 無意味） | デバイス定義の `audioIn.numChannels: 2` 宣言が**ビルド解決で 1（モノラル）に落ちる**（SDK の defines 解決順の罠。実績 2026-07-07） | `stack-chan/firmware/stackchan/manifest_breath_deploy.json` の `defines.audioIn.numChannels: 2` 明示オーバーライドを維持する。確認は build 後の `mc.defines.h` で `MODDEF_AUDIOIN_NUMCHANNELS (2)` |
...
| `Cannot find name 'Disposable'` | fork 内 `firmware/stackchan/manifest.json` の tsconfig lib 設定（`esnext.disposable`）を確認 |
| `fontbm` / libfreetype | `brew install freetype` |
| upstream merge が衝突する | `git -C stack-chan merge upstream/develop` のコンフリクトを breath ブランチ上で解消しコミット → ビルド確認 → `push origin breath` |
