...
| `mods/breath/dev/` | Wi-Fi 開発ツール（UDP trace ミラー + `GET /status` + mDNS。§Wi-Fi 開発ツール参照） |
...
- `mod.js` は呼吸ループ + ステータスバー（起動 ~2s 後）+ dev ツール（起動 ~3s 後、`config.breathDevTools` 時）。touch-debug は未接続
...
- **stack-chan の `HttpServerService` と SDK の `listen` は使わない**: 前者は任意の 1 リクエスト、後者は不正 HTTP（ポートスキャン等）で unhandled rejection → XS abort → 再起動する（`docs/tasks/open-items.md` Phase 1 実装メモ参照）。dev-server は rejection ハンドラを補修した overlay コピー `breath/dev/http-listen` を使用
