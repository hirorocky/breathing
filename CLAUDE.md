...
| `mods/breath/dev/` | Wi-Fi 開発ツール（UDP trace ミラー + `GET /status` + `PUT /ota` + mDNS。§Wi-Fi 開発ツール・§Wi-Fi OTA デプロイ参照） |
...
`overlay/mods/breath/dev/`（trace-udp / dev-server / dev-tools）。`manifest_breath_deploy.json` の `"breathDevTools": true` で有効。
...
curl http://stackchan.local/status   # mDNS が通らないネットワークでは IP 直打ち
curl http://192.168.68.76/status
```
...
- `stackchan.local` は自宅 Deco IoT SSID ではクライアント間 multicast 遮断のため解決不可（デバイス側 mDNS は正常）。IP 直打ちで代用
...
overlay/scripts/ota-deploy.sh 192.168.1.50   # ホストを明示（IP 直打ち。mDNS は本 LAN では不可）
```

やること: buildId 付き build → `PUT /ota`（`x-dev-token: breath-dev`）→ 再起動待ち → `GET /status` の buildId 照合。USB には触らない。
...
| `/status` も ping も UDP ログも無反応 | **まず電源と IP を疑う**（DHCP で IP は変わる。実績: .76 → .66）。クラッシュとは限らない | 電源を確認 → `logs.sh` を起動した状態で電源投入 → UDP ブートトレースの送信元 IP が新 IP。恒久対策はルーターの DHCP 予約 |
