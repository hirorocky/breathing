...
// 既存どおり: 間引き purge で窓外行を掃除（下記）
count = COUNT(*) WHERE last_seen_at >= cutoff
```

### 確定（archive）のタイミング

次の **2 経路**で同じ `archive_visit` を呼ぶ（冪等に近いよう visit ごとに UUID を新規発行）。

| 経路 | きっかけ |
|---|---|
| A. 再訪問 | 同 `session_id` で窓外後に再度 presence が来た |
| B. 定期 purge | `shouldPurgePresence(now)` が true のとき、窓外の `active_sessions` を走査 |

B により、再訪問しないユーザーの visit も最大約 300 秒 + purge 間隔で残る。
...
期限切れ visit は `DELETE FROM session_visits WHERE expires_at < ?` を words 削除と同様に POST / words または purge 時に間引き実行。
...
実装済み: `0004_session_visits.sql`, `worker/src/db/visits.ts`, `worker/src/routes/public.ts`, `worker/src/routes/admin.ts`（`session-stats`）, `app/privacy/page.tsx`
