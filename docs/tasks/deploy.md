...
静的サイト（Pages）と API（Worker）が同一ドメインで動作している。以下は初回構築時の手順記録と、再デプロイ・確認用の参照。

## 本番環境（現在）

| 項目 | 値 |
|---|---|
| 本番 URL | https://breathing-4k7.pages.dev |
| GitHub | https://github.com/hirorocky/breathing |
| Pages プロジェクト | `breathing`（`breathing-4k7.pages.dev`） |
| Worker | `breathing-api` |
| D1 | `breathing`（`database_id` は `worker/wrangler.toml`） |
| `/api` 接続 | §9B（Pages Functions → Service binding `BREATHING_API`） |
| `ALLOWED_ORIGINS` | `https://breathing-4k7.pages.dev` |
| D1 マイグレーション | `0001` 〜 `0004` 適用済み |
| 定期バッチ | Workers Cron `*/5 * * * *`（visit 確定・期限削除） |

`ADMIN_TOKEN` と `CLOUDFLARE_API_TOKEN` は GitHub / リポジトリに含めない。
...
- [x] §1 ローカルでビルド確認
- [x] §2 GitHub リポジトリ作成・初回 push

**フェーズ 2 — Cloudflare Worker（GitHub 不要）**

- [x] §3 Account ID・API トークン
- [x] §4 D1 作成・`wrangler.toml` 更新・migrate
- [x] §5 Worker 初回デプロイ・`ADMIN_TOKEN`

**フェーズ 3 — GitHub と Cloudflare を接続**

- [x] §6 `database_id` の変更を commit / push
- [x] §7 GitHub Secrets 登録
- [x] §8 Cloudflare Pages を Git に接続・初回ビルド

**フェーズ 4 — 公開の仕上げ**

- [x] §9 ドメイン設定（`pages.dev`・§9B）
- [x] §10 `ALLOWED_ORIGINS` 設定・Worker 再デプロイ
- [x] §11 動作確認
...
> **完了メモ:** リポジトリ `hirorocky/breathing`、`main` で CI 稼働中。
...
> **完了メモ:** `breathing-api` デプロイ済み。以降の schema 変更は `npm run db:migrate:remote` または Actions の migrate ステップで適用する。
...
> **完了メモ:** 本番 URL は https://breathing-4k7.pages.dev 。`NEXT_PUBLIC_ONLINE=1` 設定済み。
...
### 9B. `*.pages.dev` のみの場合（**本番はこちら**）

§8 で得た URL（本番: `https://breathing-4k7.pages.dev`）を本番 URL とする。
...
- [x] トップが表示される
- [x] 人数が表示される（0 でも可。常に 3〜5 なら API 未接続の fallback）
- [x] キー入力で言葉を置ける
- [x] 言葉が自分の画面に漂う
- [x] 「この場について」→ `/privacy`
...
```bash
curl -s "https://breathing-4k7.pages.dev/api/presence"
curl -s -X POST "https://breathing-4k7.pages.dev/api/words" \
...
curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
  "https://breathing-4k7.pages.dev/api/admin/session-stats"
```

### GitHub

- [x] **CI** が green
- [x] `main` 更新で Pages が再ビルドされる
- [x] **Deploy Worker** が成功する
...
| D1 マイグレーション追加時 | `worker/migrations/` を push → Actions が `db:migrate:remote` |

API 緊急停止: `STATIC_ONLY_MODE = "true"` → commit / deploy。

visit 記録の確定・期限削除は Workers Cron（5 分間隔）が担当。手動確認は §11 の `session-stats`。
