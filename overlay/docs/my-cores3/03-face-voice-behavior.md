...
| ファイル | 役割 |
|---|---|
| `renderer-breath.ts` | `BreathFace` を StackChan の Renderer 契約へ接続 |
| `breath-face.ts` | 320 × 240 画面上に左右の目を配置し、まばたきを構成 |
| `eye-cozmo.ts` | 角丸矩形の目、呼吸脈動、視線、漂い、表情用まぶたを描画 |
...
感情状態は `emotion.ts` が管理し、`eye-cozmo.ts` の上まぶた表現へ接続します。顔の形や感情表現を変える場合は、標準 renderer ではなくこの3ファイルと `emotion.ts` の接続を確認します。

## 声

breathing では文章読み上げや TTS を使わず、`overlay/mods/breath/cry.ts` の短い非言語音だけを使います。
...
鳴き声のレシピを変更するときは、端末実装 `cry.ts` と試作用 `overlay/tools/cry/synth.py` の対応を確認します。文章発話、外部 TTS、リップシンクは breath の実装経路ではありません。

## 呼吸と生存感

`mod.ts` の呼吸ループは吸気4秒・呼気6秒を基準に小さく揺らし、目の脈動へ渡します。`liveliness.ts` は通常呼吸を置き換えず、視線の微揺らぎ、深呼吸、まれな murmur を重ねます。

`emotion.ts` は valence / arousal を保持し、呼吸速度、顔、姿勢、LED の修飾値を提供します。状態変化を追加するときは、一つのイベントから複数表現を直接操作せず、感情状態または各担当モジュールの公開 API を介して接続します。

## 音への反応

`mic.ts` はステレオマイクから短い窓のレベルと特徴量を計算し、`loud`、`clap`、`voice`、`silence` などのイベントを通知します。音声内容は扱いません。

`reactions.ts` はイベントを購読し、驚き、方向付きの一瞥、反応抑制を担当します。新しい反応は連続発火、マイク再開、鳴き声やサーボとの競合を考慮してここへ追加します。

## 視線と姿勢

- 視線: `liveliness.ts` と `reactions.ts` が `robot.lookAt()` / `lookAway()` を使う
- 首の視線追従: StackChan 本体の `updatePose()` が `gazeServoFollowDeg` の閾値を超えた注視方向へ追従する
- 感情姿勢: `posture.ts` が pitch を低頻度かつゆっくり変更する
- 驚き: `posture.ts` の recoil が一時的に頭を動かす
...
`led.ts` はヘッド LED を感情の環境光として使い、呼吸に合わせて明るさをわずかに変化させます。強い通知や状態表示には使わず、顔・姿勢・音と同様に静かな同席感を優先します。
