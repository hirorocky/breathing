...
import type { GazeRobot } from 'breath/types'
...
let gazeTimerId: ReturnType<typeof Timer.set> | null = null
let gazeSettleTimerId: ReturnType<typeof Timer.set> | null = null
let murmurTimerId: ReturnType<typeof Timer.set> | null = null
 ...
 */
function mergeValidated<T>(base: T, patch: unknown): T {
...
function sanitizeParams<T extends typeof defaults>(target: T): T {
  for (const path of Object.keys(CLAMP_RANGES)) {
    const [section, key] = path.split('.')
    if (!section || !key) continue
    const [min = Number.NEGATIVE_INFINITY, max = Number.POSITIVE_INFINITY] =
      CLAMP_RANGES[path as keyof typeof CLAMP_RANGES] ?? []
    const targetRecord = target as unknown as Record<string, Record<string, unknown>>
    const sectionRecord = targetRecord[section]
    const value = sectionRecord?.[key]
    if (sectionRecord && typeof value === 'number') {
      sectionRecord[key] = Math.min(max, Math.max(min, value))
    }
  }
  return target
}
...
function poissonIntervalMs(meanMs: number, minMs: number) {
...
function scheduleNextGaze() {
  if (!started) return
  const cfg = params.gaze
  // biome-ignore lint/suspicious/noUnnecessaryConditions: enabled is mutable through runtime parameter updates.
  if (!cfg.enabled) {
...
export function startLiveliness(robot: GazeRobot) {
  if (started) return
  started = true
  robotRef = robot
  loadPersistedParams()
 ...
 */
export function deferGaze(ms: number) {
  if (!started) return
  clearGazeTimers()
  gazeTimerId = Timer.set(() => {
    gazeTimerId = null
 ...
 */
export function setParams(partial: unknown) {
