	...
	new HTTPServer({ 
		...
		onConnect(_connection_) {
			let requestPromise = new Promise((resolveRequest, rejectRequest) => {
				...
				// breathing 修正: パース失敗など onRequest 前の接続エラーで onError が
				// reject したとき、ハンドラ未接続の rejection が XS abort（再起動）になる
				// のを防ぐ。後続の await responsePromise には影響しない。
				responsePromise.then(undefined, () => {});
			});
			// breathing 修正: 同上（requestPromise 側）。
