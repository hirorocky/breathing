...
import { deepClone, loadParams, mergeValidated, persistParams } from 'breath/param-store'
import type { EmotionEvent, LedRobot, MicEvent } from 'breath/types'
...
let tickTimerId: ReturnType<typeof Timer.repeat> | null = null
...
function gamma01(t: number) {
  return (GAMMA[Math.round(clamp01(t) * 255)] ?? 0) / 255
}

function clampLedParams<T extends typeof defaults>(target: T): T {
...
function rebuildLayoutMap() {
  ...
  const split = Math.trunc(params.layout?.split ?? 0)
...
function lerpColorInto(from: Color, to: Color, t: number, out: Color) {
...
function computeHueColorInto(v: number, out: Color) {
  if (v > 0) {
    const t = clamp(v, 0, 1)
...
function setFrameLed(physIndex: number, hue: Color, value: number) {
...
function maxFrameLed(physIndex: number, hue: Color, value: number) {
  const m = Math.max(hue.r, hue.g, hue.b)
  if (m <= 0 || value <= 0) return
  const s = value / m
  const o = physIndex * 3
  ...
  if (r > (frame[o] ?? 0)) frame[o] = r
  if (g > (frame[o + 1] ?? 0)) frame[o + 1] = g
  if (b > (frame[o + 2] ?? 0)) frame[o + 2] = b
}
...
function currentWidth(a: number) {
 ...
 */
function litValue(e: number, ceil: number, floor: number) {
  if (e <= 0) return 0
...
function updateHueTransition(now: number, targetV: number) {
  if (transActive) {
...
function hueAt(p: number, out: Color) {
  if (!transActive) {
...
function renderSleepFrame(now: number) {
  frame.fill(0)
...
function overlayEffects(now: number) {
  ...
  if (effectType === EFFECT_STARTLE) {
    ...
    for (let p = 0; p < LED_COUNT; p++) {
      ...
      if (o > 0) maxFrameLed(layoutMap[p] ?? 0, hueScratch, value * o)
    }
    return
  }
  ...
  for (let p = 0; p < LED_COUNT; p++) {
    ...
    if (o > 0) maxFrameLed(layoutMap[p] ?? 0, TOUCH_HUE, value * o)
  }
}

function renderAmbientFrame(now: number) {
  let v = 0
  let a = 0
  let sleepy = false
  try {
  ...
  for (let p = 0; p < LED_COUNT; p++) {
    ...
    if (value <= 0) continue
    hueAt(p, hueScratch)
    setFrameLed(layoutMap[p] ?? 0, hueScratch, value)
  }

  overlayEffects(now)
}

function renderSweepFrame(now: number) {
...
function handleMicEvent(event: MicEvent) {
  try {
    ...
    if (event?.type !== 'loud' && event?.type !== 'clap') return
...
function handleEmotionEvent(event: EmotionEvent) {
  try {
    if (!(started && params.enabled)) return
...
export function startLed(robot: LedRobot) {
  if (started) return
  started = true
  params = clampLedParams(loadParams(PREF_KEY, defaults, {}))
  rebuildLayoutMap()

  let head: { off: () => void } | null = null
  try {
...
export function stopLed(robot: LedRobot | null | undefined) {
  started = false
  if (tickTimerId) {
...
export function getLedStatus() {
  ...
  let best = -1
  let bi = 0
  const frameOut: number[][] = []
  for (let i = 0; i < LED_COUNT; i++) {
    const o = i * 3
    const s = (frame[o] ?? 0) + (frame[o + 1] ?? 0) + (frame[o + 2] ?? 0)
    if (s > 0) lit++
    if (s > best) {
      best = s
      bi = o
    }
    frameOut.push([frame[o] ?? 0, frame[o + 1] ?? 0, frame[o + 2] ?? 0])
  }
...
export function setLedParams(partial: unknown) {
 ...
 */
export function setLedSingle(index: number, r: number, g: number, b: number, ms: number) {
 ...
 * 総所要時間(ms)を返す。0 は失敗。
 */
export function startLedSweep(ms: number) {
 ...
 */
export function testLed(r: number, g: number, b: number, ms: number) {
  if (!expander) return false
  try {
